...
- 规划任务由 `CreatePlanningTaskPacket` 创建：服务端校验规划师在岗、方块 id 有效、体积上限、城市资金，并按体积一次性预扣资金(`ServerConfig.plannerMoneyPerBlock`，可在配置 GUI 调整)。填充/替换的真实方块来自紧贴建筑盒的箱子，清除掉落物入该箱；基岩/建筑盒/容器受保护。
